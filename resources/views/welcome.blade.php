...
<body class="bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center gap-4">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="px-4 py-2 rounded-lg bg-gradient-primary text-white font-medium hover:shadow-lg transition-all">
                            لوحة التحكم
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:text-purple-600 font-medium transition">
                            {{ __('Login') }}
                        </a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="px-4 py-2 rounded-lg bg-gradient-primary text-white font-medium hover:shadow-lg transition-all">
                                {{ __('Register') }}
                            </a>
                        @endif
                    @endauth
    ...
    <section class="min-h-screen flex items-center justify-center px-4 py-12 gradient-primary text-white">
        <div class="max-w-4xl mx-auto text-center">
            ...
            <div class="flex gap-4 justify-center flex-wrap">
                @auth
                    <a href="{{ url('/dashboard') }}" class="px-8 py-4 bg-white text-purple-600 font-bold rounded-lg hover:shadow-2xl transition-all text-lg">
                        ⚡ الذهاب إلى لوحة التحكم
                    </a>
                @else
                    <a href="{{ route('login') }}" class="px-8 py-4 bg-white text-purple-600 font-bold rounded-lg hover:shadow-2xl transition-all text-lg">
                        🔐 تسجيل الدخول
                    </a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="px-8 py-4 border-2 border-white text-white font-bold rounded-lg hover:bg-white/10 transition-all text-lg">
                            ✨ إنشاء حساب جديد
                        </a>
                    @endif
                @endauth
    ...
    <section class="py-20 px-4 bg-white dark:bg-gray-800">
        <div class="max-w-4xl mx-auto text-center">
            ...
            <div class="flex gap-4 justify-center flex-wrap">
                @auth
                    <a href="{{ url('/dashboard') }}" class="px-8 py-4 gradient-primary text-white font-bold rounded-lg hover:shadow-2xl transition-all text-lg">
                        الذهاب إلى لوحة التحكم
                    </a>
                @else
                    <a href="{{ route('login') }}" class="px-8 py-4 gradient-primary text-white font-bold rounded-lg hover:shadow-2xl transition-all text-lg">
                        تسجيل الدخول الآن
                    </a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="px-8 py-4 border-2 border-purple-600 text-purple-600 dark:text-purple-400 dark:border-purple-400 font-bold rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-all text-lg">
                            إنشاء حساب جديد
                        </a>
                    @endif
                @endauth
    ...
    <footer class="bg-gray-900 text-white py-12 px-4">
        {{-- <div class="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
            <div>
                <h4 class="text-xl font-bold mb-4">عن النظام</h4>
                <p class="text-gray-400">منصة احترافية لإدارة المستخدمين والقطاعات التعليمية</p>
            </div>
            <div>
                <h4 class="text-xl font-bold mb-4">الروابط السريعة</h4>
                <ul class="text-gray-400 space-y-2">
                    <li><a href="#" class="hover:text-white transition">الرئيسية</a></li>
                    <li><a href="#" class="hover:text-white transition">حول</a></li>
                    <li><a href="#" class="hover:text-white transition">تواصل</a></li>
                </ul>
            </div>
            <div>
                <h4 class="text-xl font-bold mb-4">المساعدة</h4>
                <ul class="text-gray-400 space-y-2">
                    <li><a href="#" class="hover:text-white transition">الدعم</a></li>
                    <li><a href="#" class="hover:text-white transition">التوثيق</a></li>
